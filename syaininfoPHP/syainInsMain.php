...
switch ($cmdBtnNo) {
    case 1: //登録ボタン
        /* 入力データをBeansにセット */
        $syainBeans->setSyainBangou($syainBangou);
        $syainBeans->setSyainMei($syainMei);

        /* DB接続 */
        $pdo = $utilConnDB->connect();

        /* SQL文実行 */
        $recCount = $syainSQL->insert($pdo, $syainBeans);
        if($recCount == 1) {
           $utilConnDB->commit($pdo);//コミット
           $syainBeans->syainBeansClear();
        }else{
           $utilConnDB->rollback($pdo);
           $syainBeans->setSyainMei("社員番号が登録済みです");
        }
...
/* 次に実行するモジュール */
header('Location: syainInsMenu.php');
?>